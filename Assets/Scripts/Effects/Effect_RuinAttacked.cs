...
[CreateAssetMenu(menuName = "Abilities/Effects/Ruin Attacked")]
public class Effect_RuinAttacked : CardEffect
{
    public override IEnumerator Run(BoardTile tile)
    {
        ...
        for (int i = 0; i < 49; i++)
        {
            if (tile.Piece.Effect[i])
            {
                ...
                {
                    target.BreakOn--;
                    Destroy(GameObject.Instantiate(VFXRegistry.Instance.Ruin, target.gameObject.transform.position + new Vector3(0, 0, -0.1f), Quaternion.Euler(90, 0, 0)), 0.5f);
                }
            }
        }
